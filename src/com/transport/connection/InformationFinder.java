...
public class InformationFinder {
    ...
    private String code = "HAH020503Ah";

    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                try {
                    InformationFinder window = new InformationFinder();
    ...
    public void searchEmployees(String searchText) {
        try {
            ...
            String query = "SELECT * FROM employees WHERE firstName LIKE ? OR lastName LIKE ? OR employeeNumber LIKE ? OR extension LIKE ? OR email LIKE ? OR officeCode LIKE ? OR reportsTo LIKE ? OR jobTitle LIKE ? OR Role LIKE ?";
    ...
    private void openEditDialog() {
        if (selectedRowIndex != -1) {
            ...
            String[] columnNames = {
                ...
                "Extension", "Email", "Office Code", "Reports To", "Job Title", "Role"
            };
            ...
            if (result == JOptionPane.OK_OPTION) {
                ...
                // You will need to write code here to update the data in your database
                int employeeID = (int) tableModel.getValueAt(selectedRowIndex, 0);
    ...
    private void openAddDialog() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            // You will need to write code here to add the data to your database
            insertData(newRow);
        }
    ...
    private void deleteData(int employeeID) {
        ...
        try {
            con = DriverManager.getConnection(dbUrl, user, code);
    ...
    private void updateData(int employeeID, String[] editedValues) {
        ...
        try {
            con = DriverManager.getConnection(dbUrl, user, code);
            String updateQuery = "UPDATE employees SET firstName = ?, lastName = ?, employeeNumber = ?, extension = ?, email = ?, officeCode = ?, reportsTo = ?, jobTitle = ?, Role = ? WHERE employeeID = ?";
    ...
    private void insertData(Vector<Object> newRow) {
        ...
        try {
            con = DriverManager.getConnection(dbUrl, user, code);
            String insertQuery = "INSERT INTO employees (firstName, lastName, employeeNumber, extension, email, officeCode, reportsTo, jobTitle, Role, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
